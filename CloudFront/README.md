...
- When a user visits wesite from a different location far away from your server, the content might take a long time to load. That's where CDN comes to the rescue!
...
- When a user requests an image, the request goes to CloudFront first, then:
...
If a client have an e-commerce website that sells products globally. By using CloudFront, the product images and videos load quickly for its customers all over the world, improving the shopping experience.

### Scenario 2: Media Streaming
With CloudFront, you can stream videos to users efficiently, regardless of their location, without buffering issues.
...
CloudFront improves the website's performance, making users happier and potentially boosting your application and business.
